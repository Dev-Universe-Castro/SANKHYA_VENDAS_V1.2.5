...
export default function UserModal({ isOpen, onClose, onSave, user, mode }: UserModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Fazer prefetch para atualizar dados instantaneamente
      console.log("🔄 Atualizando cache de usuários...")
      toast({
        title: "Atualizando dados...",
        description: "Sincronizando informações dos usuários",
      })

      try {
        const prefetchResponse = await fetch('/api/prefetch', { 
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          }
        ...
        if (prefetchResponse.ok) {
          ...
          if (prefetchData.success !== false) {
            ...
            console.warn("⚠️ Prefetch retornou erro, mas usuário foi salvo")
          }
        }
      } catch (prefetchError) {
        console.warn("⚠️ Erro ao atualizar cache, mas usuário foi salvo:", prefetchError)
        toast({
          title: "Atenção",
          description: "Usuário salvo, mas cache pode estar desatualizado. Recarregue a página.",
          variant: "default",
        })
      }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative bg-card rounded-lg shadow-lg w-full max-w-2xl md:p-6 p-0 max-h-[95vh] h-full md:h-auto flex flex-col overflow-hidden">
        {/* Header - Desktop */}
        <div className="hidden md:flex items-center justify-between mb-6 flex-shrink-0">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
          <div className="space-y-4 p-4 md:p-0 overflow-y-auto flex-1 pb-2 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          {/* Avatar Preview - Desktop Only */}
          <div className="hidden md:flex flex-col items-center gap-3 pb-2 border-b flex-shrink-0">
          ...
          <div className="flex gap-3 py-4 px-4 md:px-0 flex-shrink-0 border-t bg-card md:bg-transparent">
            <Button 
              type="button" 
              variant="outline" 
              onClick={onClose} 
