...
export default function PedidosFDVTable() {
        ...
        if (codParc) {
          try {
            ...
            if (parceiro) {
              ...
              return {
                ...p,
                NOME_PARCEIRO_DB: novoNome // Campo auxiliar para exibição
              };
            }
          } catch (e) {
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50/30 scrollbar-hide">
      <div className="flex-1 overflow-auto scrollbar-hide">
        <div className="p-0 md:p-6">
          <div className="bg-white border-b md:border md:rounded-xl shadow-sm overflow-hidden">
            <div className="px-4 py-4 md:px-6 md:py-4 border-b bg-white">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <Button variant="outline" size="sm" onClick={() => setShowFilters(!showFilters)} className={`h-10 px-4 text-xs font-semibold rounded-lg flex items-center gap-2 transition-all ${showFilters ? 'bg-green-50 border-green-200 text-green-700' : 'border-gray-200 text-gray-600'}`}>
                    <Filter className="w-4 h-4" />
                    <span>Filtros</span>
                    {(filtroOrigem !== 'TODOS' || filtroStatus !== 'TODOS' || filtroDataInicio || filtroDataFim || filtroParceiro) && <div className="w-2 h-2 rounded-full bg-green-600 animate-pulse" />}
                  </Button>
                </div>
                <Button onClick={() => setShowNovoPedidoModal(true)} className="w-full md:w-auto bg-green-600 hover:bg-green-700 h-12 md:h-10 font-bold rounded-lg shadow-md shadow-green-200 transition-all active:scale-[0.98]">
            ...
            <div className={`overflow-hidden transition-all duration-300 ease-in-out bg-gray-50/50 ${showFilters ? 'max-h-[500px] opacity-100 border-b' : 'max-h-0 opacity-0'}`}>
            ...
            <div className="bg-white">
              <div className="md:hidden divide-y divide-gray-100">
                {loading ? (
                  ...
                  <div className="py-20 text-center px-6">
                    ...
                    pedidos.map((pedido) => (
                    <div key={pedido.ID} className="p-4 active:bg-gray-50 transition-colors border-b last:border-0">
                ...
                <Table>
                  ...
                  <TableBody>
                    {loading ? (
                      ...
                      <TableRow><TableCell colSpan={8} className="text-center py-12 text-gray-500">Nenhum pedido encontrado</TableCell></TableRow>
                    ) : (
                      pedidos.map((pedido) => (
              ...
              <div className="px-4 py-4 md:px-6 border-t bg-gray-50/50">
