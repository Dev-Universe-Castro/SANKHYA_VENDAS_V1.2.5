...
import { Home, Users, Calendar, ShoppingCart, DollarSign, Package, UserCircle, Settings, Download, Loader2, Cloud, CloudOff, LogOut, LayoutGrid, Route, Table } from "lucide-react"
...
export function MenuBottomSheet({ open, onOpenChange }: MenuBottomSheetProps) {
  ...
  const allMenuItems = useMemo(() => {
    ...
    if (isAdmin || screens.telaPedidosVendas) {
      items.push({ href: "/dashboard/pedidos", label: "Pedidos", icon: ShoppingCart })
    }
    ...
    if (isAdmin || screens.telaTabelaPrecos) {
      items.push({ href: "/dashboard/tabelas-precos", label: "Tab. Preços", icon: Table })
    }
