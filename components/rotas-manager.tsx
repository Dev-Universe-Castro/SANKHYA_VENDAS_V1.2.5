    ...
    <div className="flex flex-col h-full bg-background overflow-hidden scrollbar-hide">
      ...
      <div className="md:hidden border-b px-3 py-3 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="flex items-center justify-between mb-2">
          <h1 className="text-lg font-bold">Rotas</h1>
          <Button onClick={() => setModalNovaRota(true)} className="bg-green-600 hover:bg-green-700 h-8" size="sm">
      ...
      <Tabs defaultValue="rotas" className="flex-1 flex flex-col overflow-hidden">
        <div className="border-b px-4 md:px-6 py-2">
          <TabsList className="grid w-full grid-cols-2 h-10 p-1 bg-gray-100/80 rounded-lg">
            <TabsTrigger value="rotas" className="text-xs sm:text-sm font-semibold transition-all data-[state=active]:bg-background data-[state=active]:shadow-sm">
              <span className="hidden sm:inline">Minhas Rotas</span>
              <span className="sm:hidden">ROTAS</span>
            </TabsTrigger>
            <TabsTrigger value="visitas" className="text-xs sm:text-sm font-semibold transition-all data-[state=active]:bg-background data-[state=active]:shadow-sm">
        ...
        <TabsContent value="rotas" className="flex-1 overflow-auto m-0 p-0 md:p-6 scrollbar-hide">
          <div className="px-4 md:px-0 space-y-4 md:space-y-6 pb-24 md:pb-6 mt-4 md:mt-0">
            {rotas.length === 0 ? (
              <div className="py-12 text-center">
                <Route className="w-12 h-12 text-muted-foreground/50 mx-auto mb-3" />
                <p className="text-sm font-medium text-muted-foreground">Você ainda não criou nenhuma rota</p>
              </div>
            ) : (
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {rotas.map((rota) => (
                  <Card key={rota.CODROTA} className="overflow-hidden border-slate-200 hover:border-green-300 transition-all shadow-sm">
                    <CardContent className="p-4">
                      ...
                      <Button 
                        className="w-full bg-green-600 hover:bg-green-700 h-9 font-bold shadow-sm text-sm"
                        onClick={() => {
        ...
        <TabsContent value="visitas" className="flex-1 overflow-auto m-0 p-0 md:p-6 scrollbar-hide">
          <div className="px-4 md:px-0 space-y-4 mt-4 md:mt-0 pb-24 md:pb-6">
                ...
                <CollapsibleTrigger asChild>
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-semibold text-sm">Filtros de Busca</span>
                    </div>
              ...
              {visitasFiltradas.length === 0 ? (
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {visitasFiltradas.map((visita) => {
                    ...
                    return (
                      <Card key={visita.CODVISITA} className={`overflow-hidden border-slate-200 transition-all shadow-sm ${isVisitaEmAndamento ? 'ring-2 ring-green-500 border-transparent bg-green-50/30' : 'hover:border-green-300'}`}>
                        <CardContent className="p-4">
                          <div className="flex items-start justify-between mb-3">
                    <div className="flex-1 min-w-0 flex gap-3">
                      <div className={`w-8 h-8 rounded-full ${isVisitaEmAndamento ? 'bg-green-100 text-green-600' : 'bg-slate-100 text-slate-500'} flex items-center justify-center font-bold shrink-0 text-xs`}>
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 mb-3">
                          ...
                          <div className="flex gap-2">
                            {isVisitaEmAndamento ? (
                              <Button 
                                className="flex-1 bg-green-600 hover:bg-green-700 h-9 font-bold shadow-sm text-xs"
                                onClick={() => {
      ...
      {/* Modais - Mantendo os mesmos mas garantindo padrão de cores */}
      <Dialog open={modalDetalhesRota} onOpenChange={setModalDetalhesRota}>
        <DialogContent className="sm:max-w-5xl h-full sm:h-[90vh] flex flex-col p-0 overflow-hidden bg-white sm:rounded-xl border-none shadow-2xl rounded-none">
          <DialogHeader className="p-4 border-b bg-slate-50/50 sticky top-0 z-10 shrink-0">
            <div className="flex items-center justify-between">
              <DialogTitle className="text-xl font-bold flex items-center gap-2">
                <Route className="w-5 h-5 text-green-600" />
                {rotaSelecionada?.DESCRICAO}
              </DialogTitle>
              <Button variant="ghost" size="icon" className="rounded-full hover:bg-slate-100 transition-colors" onClick={() => setModalDetalhesRota(false)}>
                <X className="h-4 w-4" />
              </Button>
            </div>
          </DialogHeader>
          ...
          />
            ...
            <div className="w-full md:w-1/3 flex flex-col bg-slate-50/30 overflow-hidden">
              ...
              <div className="p-4 bg-white border-t sticky bottom-0">
                <Button 
                  className="w-full bg-green-600 hover:bg-green-700 h-10 font-bold shadow-md"
                  onClick={() => {
      ...
      <Dialog open={modalNovaRota} onOpenChange={setModalNovaRota}>
        <DialogContent className="max-w-4xl max-h-[95vh] flex flex-col p-0 overflow-hidden bg-white rounded-xl border-none shadow-2xl">
                        ...
                        >
                          <div className="flex items-center gap-3 min-w-0">
                            <div className={`w-5 h-5 rounded border flex items-center justify-center transition-all ${
                              isSelected ? 'bg-green-600 border-green-600 shadow-sm' : 'border-slate-300 bg-white'
                            }`}>
          ...
          <DialogFooter className="p-4 border-t bg-slate-50/80 sticky bottom-0 z-10">
             <div className="flex items-center justify-between w-full">
                <Button variant="outline" onClick={() => setModalNovaRota(false)} className="h-10 px-6 border-slate-300 font-bold text-slate-600 bg-white">
                  Cancelar
                </Button>
                <Button 
                  onClick={criarRota} 
                  disabled={!novaRotaForm.descricao || novaRotaForm.parceiros.length === 0 || criandoRota}
                  className="h-10 px-8 bg-green-600 hover:bg-green-700 text-white font-bold shadow-lg shadow-green-600/20"
                >
                  {criandoRota ? (
                    <>
      ...
      <Dialog open={modalCheckin} onOpenChange={setModalCheckin}>
        <DialogContent className="sm:max-w-md h-full sm:h-auto flex flex-col p-0 overflow-hidden bg-white sm:rounded-xl border-none shadow-2xl rounded-none">
          <DialogHeader className="p-4 border-b bg-slate-50/50 sticky top-0 z-10 shrink-0">
            <div className="flex items-center justify-between">
              <DialogTitle className="text-xl font-bold flex items-center gap-2">
          ...
          <DialogFooter className="p-6 bg-slate-50/30 border-t flex flex-col sm:flex-row gap-3 mt-auto">
            <Button variant="outline" onClick={() => setModalCheckin(false)} className="h-11 font-bold flex-1 border-slate-300">
              Agora não
            </Button>
            <Button onClick={fazerCheckin} className="bg-green-600 hover:bg-green-700 h-11 font-bold flex-1 shadow-md shadow-green-600/10">
      ...
      <Dialog open={modalCheckout} onOpenChange={setModalCheckout}>
        <DialogContent className="sm:max-w-lg h-full sm:h-auto flex flex-col p-0 overflow-hidden bg-white sm:rounded-xl border-none shadow-2xl rounded-none">
          <DialogHeader className="p-4 border-b bg-slate-50/50 sticky top-0 z-10 shrink-0">
            <div className="flex items-center justify-between">
              <DialogTitle className="text-xl font-bold flex items-center gap-2">
              ...
              <Button variant="ghost" size="icon" className="rounded-full hover:bg-slate-200 transition-colors" onClick={() => setModalCheckout(false)}>
                <X className="h-4 w-4" />
              </Button>
            </div>
          </DialogHeader>
          ...
          <DialogFooter className="p-6 bg-slate-50/30 border-t flex flex-col sm:flex-row gap-3 mt-auto">
            <Button variant="outline" onClick={() => setModalCheckout(false)} className="h-11 font-bold flex-1 border-slate-300">
              Continuar Visita
            </Button>
            <Button onClick={fazerCheckout} className="bg-green-600 hover:bg-green-700 h-11 font-bold flex-1 shadow-lg shadow-green-600/10">
      ...
      <Dialog open={modalMapaVisita} onOpenChange={setModalMapaVisita}>
        <DialogContent className="sm:max-w-3xl h-full sm:h-[80vh] flex flex-col p-0 overflow-hidden bg-white sm:rounded-xl border-none shadow-2xl">
      ...
      <Dialog open={modalAtividade} onOpenChange={setModalAtividade}>
        <DialogContent className="sm:max-w-md bg-white rounded-xl">
          ...
          <DialogFooter>
            ...
            <Button onClick={criarAtividadeVinculada} className="bg-green-600 hover:bg-green-700">Salvar Atividade</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
